...
# ⚡ 局部重绘引擎 (极致卡牌交互体验，PVE打怪/抽签绝不引起全局刷新)
try:
...
# ==============================================================================
# 🌌 [ GLOBALS ] TCG 引擎状态机与无名逆流版权
# ==============================================================================
VERSION = "KARMA TCG V110.0 [THE NAMELESS GENESIS]"
COPYRIGHT = "无名逆流"
SYS_NAME = "量子卡牌 | 神之局"
...
if "active_buff" not in st.session_state: st.session_state["active_buff"] = {"atk_mul": 1.0, "def_mul": 1.0, "name": "无"}
if "raid_state" not in st.session_state: 
    st.session_state["raid_state"] = {"lvl": 1, "boss_hp": 150000, "boss_max": 150000, "boss_atk": 8000, "player_hp": 0, "player_max": 0, "logs": ["> AWAITING RAID INITIATION..."]}
...
.relic-item { background: rgba(0,0,0,0.6); border: 1px solid #333; border-left: 3px solid currentColor; padding: 10px; border-radius: 4px; font-size: 13px; color: #fff; font-weight: bold; font-family: 'Noto Sans SC'; box-shadow: inset 0 0 10px rgba(255,255,255,0.02); transition: all 0.2s; }
...
/* PVE 动态血条 */
.hp-bar-bg { width: 100%; height: 12px; background: rgba(255,255,255,0.1); border-radius: 6px; overflow: hidden; position: relative; margin-bottom: 5px; border: 1px solid rgba(255,255,255,0.2); }
.hp-bar-fill { height: 100%; position: absolute; top: 0; left: 0; transition: width 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
...
# ==============================================================================
# 🗃️ [ TCG DICTIONARY ] 八字映射卡组与深层 Lore (全量满血回归)
# ==============================================================================
DAY_MASTER_DICT = {
    "甲": {"class_name": "Paladin / 圣骑士", "mbti": "ENTJ", "color": "#10b981", "element": "木", "tier": "UR", "base_atk": 2500, "base_def": 4000, "hp": 12000, "desc": "掌控底层因果的重装核心。能扛起重构秩序的开拓者。", "weapon": "高分子动能巨斧", "skill": "[被动] 建木庇护：受到致命伤害触发锁血。", "evo_path": "架构幼苗 ➔ 核心骨干 ➔ 苍天建木", "ult_evo": "【苍天建木】执掌三界底层协议", "flaw": "过于刚硬，遇强则折。遭遇降维打击易因宁折不弯导致宕机。", "patch": "引入水属性柔性冗余，挂起进程等待重启。"},
    "乙": {"class_name": "Assassin / 刺客", "mbti": "ENFP", "color": "#a855f7", "element": "木", "tier": "SSR", "base_atk": 3800, "base_def": 1500, "hp": 8500, "desc": "敏锐的暗网爬虫，在资源枯竭的敌方后排疯狂窃取权限。", "weapon": "量子绞杀魔藤", "skill": "[主动] 寄生吸血：每次攻击窃取敌方 15% 算力补给自身。", "evo_path": "寄生节点 ➔ 渗透猎手 ➔ 噬星魔藤", "ult_evo": "【噬星魔藤】寄生控制全网资源", "flaw": "极度依赖宿主，宿主阵亡则自身属性减半。", "patch": "采用分布式多宿主绑定协议分散风险。"},
    ...
    "丁": {"class_name": "Enchanter / 附魔师", "mbti": "INFJ", "color": "#ffaa00", "element": "火", "tier": "SSR", "base_atk": 2000, "base_def": 2500, "hp": 9000, "desc": "夜行者，在最灰暗战局中为团队提供精神增益与破防制导。", "weapon": "高聚能激光短刃", "skill": "[光环] 灵魂织网：全队获得 40% 护甲穿透增益。", "evo_path": "寻路信标 ➔ 精神图腾 ➔ 灵魂织网者", "ult_evo": "【灵魂织网者】操控全网心智的网络幽灵", "flaw": "能量波动不稳定，容易被清场 AOE 一波带走。", "patch": "需绑定甲木系主T作为遮风挡雨的掩体。"},
    "戊": {"class_name": "Fortress / 堡垒", "mbti": "ISTJ", "color": "#fcee0a", "element": "土", "tier": "UR", "base_atk": 1500, "base_def": 5000, "hp": 15000, "desc": "物理级断网防御力，最坚不可摧的矩阵底线与主T坦位。", "weapon": "绝对零度力场盾", "skill": "[被动] 盖亚装甲：免疫一次致死级降维打击。", "evo_path": "承载沙盒 ➔ 巨石阵列 ➔ 盖亚装甲", "ult_evo": "【盖亚装甲】承载万物因果的绝对壁垒", "flaw": "系统庞大笨重，面临敏捷迭代时易卡死在旧循环中。", "patch": "主动清理缓存，接纳木属性破坏性创新打破死锁。"},
    "己": {"class_name": "Summoner / 召唤师", "mbti": "ISFJ", "color": "#d4af37", "element": "土", "tier": "SSR", "base_atk": 1800, "base_def": 3800, "hp": 11000, "desc": "海纳百川的存储池。无缝整合碎片转化为冗余资源护盾。", "weapon": "引力塌缩发生器", "skill": "[主动] 内存回收：回合结束时恢复 10% 体力。", "evo_path": "容错冗余 ➔ 资源枢纽 ➔ 创世息壤", "ult_evo": "【创世息壤】孕育下一个数字生态温床", "flaw": "无差别接收并发请求导致垃圾填满超载崩溃。", "patch": "编写无情垃圾回收(GC)脚本，拒绝无效请求。"},
    ...
    "辛": {"class_name": "Sniper / 狙击手", "mbti": "INTP", "color": "#e0e0e0", "element": "金", "tier": "SSR", "base_atk": 4500, "base_def": 1800, "hp": 7500, "desc": "追求极致的微观造物主。在无形中精准切断敌方的底层协议。", "weapon": "纠缠态纳米手术刀", "skill": "[主动] 纳米解构：所有攻击无视敌方 50% 物理装甲。", "evo_path": "精密协议 ➔ 审美巅峰 ➔ 量子纠缠体", "ult_evo": "【量子纠缠体】超越物质形态的究极艺术代码", "flaw": "极度脆弱傲娇，遇粗暴环境即当场罢工。", "patch": "需要极度纯净的水属性淘洗保护，绝不卷入肮脏博弈。"},
...
PAST_LIVES = [
    ...
    {"title": "V3.0 财阀数据奴隶", "debt": "曾被困于低保底卡池。对【传说卡牌】极度渴望。"}, 
    {"title": "V4.0 赛博雇佣兵", "debt": "清除了太多中立卡。需挂载辅助技能抵消业力。"}, 
    {"title": "V5.0 矩阵先知", "debt": "偷看牌库导致规则崩坏。直觉(INT)满级，但血量减半。"}
]

# TCG 黑市可购买词条
LOOT_POOL = list(EQUIPS_DICT.values())

# TCG 每日法术卡池 & PVE Buff 联动
SPELL_POOL = [
    {"name": "✨ 乾为天 [GOD_MODE]", "type": "UR 场地魔法", "lines": [1,1,1,1,1,1], "buff_atk": 2.0, "buff_def": 1.0, "desc": "获取最高权限。今日 PVE 战斗攻击力翻倍！", "color": "#ffaa00", "do": "满仓梭哈、发起终极突击", "dont": "进入低功耗防御模式"},
    {"name": "🛡️ 坤为地 [SAFE_MODE]", "type": "SSR 永续陷阱", "lines": [0,0,0,0,0,0], "buff_atk": 0.5, "buff_def": 3.0, "desc": "进入绝对防御。今日 PVE 防御力提升 3 倍，但攻击力减半。", "color": "#10b981", "do": "冷钱包存储、挨打反伤", "dont": "开启高倍杠杆、激进输出"},
    {"name": "⚡ 地天泰 [SYNC_MAX]", "type": "UR 增益法术", "lines": [1,1,1,0,0,0], "buff_atk": 1.5, "buff_def": 1.5, "desc": "API 完美握手。今日 PVE 战斗全属性提升 50%。", "color": "#00f3ff", "do": "全网跨界融合、释放大招", "dont": "过度保守、丢弃手牌"},
    {"name": "💥 天地否 [DDOS_STRIKE]", "type": "SR 陷阱触发", "lines": [0,0,0,1,1,1], "buff_atk": 0.1, "buff_def": 0.1, "desc": "引爆全网大雪崩。今日 PVE 战力被封印至 10%！", "color": "#f43f5e", "do": "立刻拔出网线、放弃今日副本", "dont": "正面硬刚大盘、转移资产"},
    {"name": "🔄 水雷屯 [BOOT_LOOP]", "type": "R 妨碍法术", "lines": [1,0,0,0,1,0], "buff_atk": 0.8, "buff_def": 0.8, "desc": "陷入死循环代码。今日 PVE 战斗属性全面降低 20%。", "color": "#a855f7", "do": "底层重构、耐心排错", "dont": "带Bug裸奔、强行打怪"},
    {"name": "⏳ 火水未济 [COMPILING]", "type": "SR 延迟法术", "lines": [0,1,0,1,0,1], "buff_atk": 1.2, "buff_def": 0.8, "desc": "代码编译中。今日 PVE 攻击力提升 20%，防御降低 20%。", "color": "#fb923c", "do": "读条准备、保持算力输出", "dont": "打断施法、强行终止进程"}
]

# ==============================================================================
# 🧠 [ TCG ALGORITHMS ] 核心引擎与战力计算 (新增元素共鸣系统)
# ==============================================================================
def calc_tcg_stats(hash_str, wx_dict, b_atk, b_def, b_hp, equip_count):
    ...
    # 🌟 新增：五行阵营共鸣系统 (Elemental Resonance)
    reso_buff = "【无元素共鸣】"
...
def trigger_gacha_draw(spell): 
    st.session_state["gacha_drawn"] = True
...
if not is_booted:
    ...
    <div class="ticker-wrap"><div class="ticker">
        <span>TCG MATRIX V110.0 <b class="up">▲BOOSTER PACK READY</b></span>
        <span>NEW BANNER: NAMELESS EDITION <b class="ur" style="color:var(--sp);">★ SP RATE UP</b></span>
    ...
    <div style="text-align: center; margin-bottom: 25px; margin-top:5vh;">
        ...
        <h1 class="hero-title" data-text="无名逆流·神之局">无名逆流·神之局</h1><br>
        <div style="color:var(--pink); font-family:'Orbitron', sans-serif; font-size:14px; font-weight:700; letter-spacing:10px; margin-top:10px;">THE NAMELESS GENESIS V110.0</div>
    </div>
    <div class="glass-panel" style="max-width: 680px; margin: 0 auto 30px auto; border-left: 4px solid var(--sp); padding: 35px; text-align:center;">
        ...
        <div style="color:#e2e8f0; font-size: 15px; line-height: 1.8;">
            输入你的物理降临坐标，在无名逆流算力池中进行 <b style="color:#fff;">单次强力抽卡 (1x PULL)</b>。<br>
            系统将为你鉴定 <b style="color:var(--pink);">卡牌稀有度 (SR/SSR/UR/SP)</b> 与绝对战力值。<br>全量解锁你的 <b style="color:var(--primary);">主战神卡</b>、<b style="color:var(--purple);">阿卡夏大盘</b> 与 <b style="color:var(--green);">矩阵深潜玩法</b>。
        </div>
    </div>
    """
    ...
    if submit_btn:
        ...
        # 🚨 [BUG 彻底剿灭] 全面采用安全提取，多重 try-except，物理消灭 NameError
        skills = []
        try:
...
# ==============================================================================
    ...
    # 🚨【成瘾核心】动态计算最终战力：基础战力 + 后期打怪买装备附加的战力
    creds = st.session_state.get("creds", 0)
    extra_cp = st.session_state.get("extra_cp", 0)
    rs = st.session_state.get("raid_state", {})
    raid_bonus = rs.get("cp_bonus", 0)
    
    base_cp = d.get("base_cp", 50000)
    final_cp = base_cp + extra_cp + raid_bonus
    ...
    if corruption > 60: 
        corr_tag, corr_desc = "【极危】重度排异反应", "五行单极属性过载。在矩阵中极易遭遇降维打击，建议寻找互补队友双排。"
    elif corruption > 30: 
        corr_tag, corr_desc = "【临界】存在系统隐患", "底层算力分布不均。需佩戴高阶圣遗物补齐短板，防止被一波清场。"
    else: 
        corr_tag, corr_desc = "【完美】免疫精神攻击", "系统架构堪称完美，拥有极强的抗打击与自愈恢复能力，是天生的大后期核心。"
    ...
    HEADER_HTML = f"""
    <div class="ticker-wrap"><div class="ticker">
        <span>THE NAMELESS EDITION: V110.0 <b class="up">▲SYNCED</b></span>
        <span>PLAYER: {player_name} <b class="up">▲ACTIVE</b></span>
        <span>SOUL_CREDS: {creds} <b class="ur">★ LOADED</b></span>
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; border-bottom:2px solid {dm_color}; padding-bottom:15px; margin-bottom:30px;">
        <div>
            <div style="font-family:'Fira Code'; color:#aaa; font-size:12px; margin-bottom:5px; font-weight:bold;">[ SOUL WALLET: 0x{hash_id[:12]} | <span style="color:var(--sp);">CREDS: {creds:,}</span> ]</div>
    ...
    with c_right:
        # 🗄️ TCG 终极战术大厅 Tabs
        t_deck, t_raid, t_gacha, t_map, t_export = st.tabs(["🎴 卡组与设定 (DECK)", "⚔️ 矩阵深潜 (BOSS RAID)", "🎰 黑市盲盒 (SHOP)", "🌌 战局大盘 (STATS)", "💼 资产确权 (MINT)"])

        with t_deck:
            render_html("<div style='font-size:13px; color:#aaa; margin-bottom:10px;'>> 你的基础手牌由四柱源码构成，鼠标悬停可抽出查看：</div>")
            ...
            with c_d2:
                # 动态拼接所有已有装备
                sk_html = "".join([f"<div class='relic-item' style='color:{'var(--sp)' if '法术' in s else 'var(--primary)'};'>{s}</div>" for s in skills_list])
                render_html(f"""
                <div class="glass-panel" style="padding:15px; border-left-color:var(--yellow); height:210px; overflow-y:auto;">
                    ...
                    <div style="font-size:11px; color:var(--primary); font-family:Orbitron; margin-bottom:6px; font-weight:bold;">>> EQUIPPED RELICS (你的装备库)</div>
        ...
        with t_raid:
            # 🌟 PVE 经济闭环：打 Boss 掉 Creds 配合特效震动
            render_html("<div style='font-size:13px; color:#aaa; margin-bottom:15px;'>以你的主将卡属性挑战矩阵底层防火墙。<b>击杀 Boss 可获得 Soul Creds (魂币) 用于黑市抽卡！</b></div>")
            ...
            def render_raid():
                ...
                with c_pve1:
                    ...
                    <div class="glass-panel" style="text-align:center; border-color:var(--pink); padding:15px;">
                        ...
                        <div style="color:var(--pink); font-family:'Orbitron'; font-weight:900; font-size:16px;">MATRIX ICE Lv.{rs["lvl"]}</div>
                        <div style="color:#aaa; font-size:11px; margin-bottom:10px;">[ S级系统防火墙 ]</div>
                        <div class="hp-bar-bg"><div class="hp-bar-fill hp-red" style="width:{boss_hp_pct}%;"></div></div>
                        <div style="font-family:'Orbitron'; font-size:13px; color:#fff; font-weight:bold; margin-bottom:10px;">{rs["boss_hp"]:,} / {rs["boss_max"]:,} HP</div>
                    ...
                    if my_hp <= 0:
                        st.error("💀 你已在深网中阵亡。请点击底部的 [ SYS_REBOOT ] 物理重启。")
                    elif rs["boss_hp"] > 0:
                        if st.button("💥 发动骇入攻击 (ATTACK)", use_container_width=True):
                            # Player Attack
                            dmg = int((real_atk + final_cp * 0.05) * random.uniform(0.85, 1.2))
                            is_crit = random.randint(1, 100) <= f_crit
                            if is_crit:
                                dmg = int(dmg * 2.5)
                                log_msg = f"<span style='color:var(--sp); font-weight:bold;'>[CRITICAL] 触发暴击！对 ICE 造成 {dmg:,} 点毁灭伤害！</span>"
                            else:
                                log_msg = f"<span style='color:var(--primary);'>[ATTACK] 你发起了普攻，造成了 {dmg:,} 点穿透伤害。</span>"
                            
                            rs["boss_hp"] -= dmg
                            if rs["boss_hp"] <= 0:
                                rs["boss_hp"] = 0
                                reward = rs["lvl"] * 1500
                                log_msg += f"<br><br><span style='color:var(--green); font-size:16px; font-weight:bold;'>🏆 [VICTORY] 防火墙已被击穿！<br>🎁 战利品：永久提升 5000 CP，并获得 <span style='color:var(--sp)'>{reward} 魂币</span>！</span>"
                                rs["cp_bonus"] += 5000
                                st.session_state["creds"] += reward
                            else:
                                # Boss Counter-Attack
                                boss_dmg = int(rs["boss_atk"] * random.uniform(0.9, 1.1))
                                actual_dmg = max(100, int(boss_dmg - (real_def * 0.5)))
                                rs["player_hp"] -= actual_dmg
                                log_msg += f"<br><span style='color:var(--pink);'>[ICE COUNTER] 防火墙反击！装甲抵消了部分伤害，受到 {actual_dmg:,} 点真实伤害。</span>"
                                if rs["player_hp"] <= 0:
                                    rs["player_hp"] = 0
                                    log_msg += "<br><br><span style='color:var(--pink); font-size:16px; font-weight:bold;'>💀 [DEFEAT] 主将卡被粉碎！连接强制中断！</span>"
                                
                            rs["logs"].append(log_msg)
                            st.rerun()
                    else:
                        if st.button("🚀 深入下一层矩阵 (NEXT LEVEL)", use_container_width=True):
                            rs["lvl"] += 1
                            rs["boss_max"] = int(rs["boss_max"] * 1.5)
                            rs["boss_hp"] = rs["boss_max"]
                            rs["boss_atk"] = int(rs["boss_atk"] * 1.25)
                            rs["player_hp"] = f_hp # 回满血
                            rs["logs"].append(f"<br><span style='color:var(--yellow);'>> [SYS] 警告：已进入第 {rs['lvl']} 层深网。HP已恢复，侦测到更强的 ICE。</span>")
                            st.rerun()
        ...
        with t_gacha:
            ...
            with c_g1:
                ...
                def render_gacha_1pull():
                    if not st.session_state.get("gacha_drawn", False):
                        ...
                        GACHA_HTML = f"""
                        <div class="tcg-card-container card-reveal" style="max-width:280px; margin:0 auto;">
                            <div class="tcg-card" style="border-color:{sc_c}; box-shadow:0 0 40px {sc_c}66; aspect-ratio: 63/88; background:linear-gradient(0deg, rgba(0,0,0,0.95), {sc_c}22); padding:15px;">
                                <div style="text-align:center;">
                                    ...
                                    <div style="font-size:18px; font-weight:900; color:#fff; font-family:'Noto Sans SC'; margin-bottom:15px; text-shadow:0 0 10px {sc_c};">{spell_card['name']}</div>
                                    ...
                                    <div style="font-size:11px; color:var(--green); font-weight:bold; text-align:left;">[+] 宜 (DO): {spell_card.get('do','')}</div>
                                    <div style="font-size:11px; color:var(--pink); font-weight:bold; text-align:left; margin-top:4px;">[-] 忌 (DONT): {spell_card.get('dont','')}</div>
                                </div>
                            </div>
                        </div>
                        """
            ...
            with c_g2:
                render_html("<div style='color:var(--yellow); font-family:Orbitron; font-size:14px; font-weight:900; margin-bottom:15px;'>[ BLACK MARKET ] 魂币黑市与检定</div>")
                
                @st_fragment
                def render_shop():
                    render_html(f"<div class='glass-panel' style='padding:15px; border-color:var(--sp); margin-bottom:10px;'><div style='font-size:12px; color:#aaa; margin-bottom:10px;'>消耗 PVE 掉落的 Soul Creds (魂币) 抽取随机圣遗物词条，永久增加基础 CP。</div><div style='font-size:18px; font-family:Orbitron; color:var(--sp); font-weight:bold; margin-bottom:10px;'>YOUR CREDS: {st.session_state['creds']:,}</div></div>")
                    
                    if st.button("🪙 消耗 1,000 Creds 抽取盲盒", use_container_width=True):
                        if st.session_state["creds"] >= 1000:
                            st.session_state["creds"] -= 1000
                            new_relic = random.choice(LOOT_POOL)
                            st.session_state["sys_data"]["skills"].append(new_relic) # 加到技能池
                            bonus_cp = random.randint(1500, 3500)
                            st.session_state["extra_cp"] += bonus_cp # 永久加CP
                            st.success(f"🎉 抽出极品词条: {new_relic}！ 战力飙升 +{bonus_cp} CP！")
                            time.sleep(1.0)
                            st.rerun()
                        else:
                            st.error("余额不足！请前往【矩阵深潜】击杀防火墙获取 Creds！")
                render_shop()

                render_html("<div style='font-size:12px; color:#aaa; margin-bottom:10px; margin-top:10px;'>向主脑发起行动成功率检定：</div>")
                @st_fragment
                def render_dice():
                    ph = st.empty()
                    with st.form(key="dice_form", clear_on_submit=False, border=False):
                        q_input = st.text_input("📝 输入检定事件：", placeholder="e.g. 梭哈这只股票能赚吗？", label_visibility="collapsed")
                    ...
                    if sub_q:
                        if not q_input: ph.warning("⚠️ 语法错误：事件为空！")
                            ...
                            <div class="glass-panel card-reveal" style="margin-top:10px; border-color:{q_c}; text-align:center; border-left-width: 4px;">
                                ...
                                <div style="font-size:13px; font-weight:bold; color:#fff; margin-bottom:5px;">判定：{hex_res['name']}</div>
        ...
        with t_map:
            ...
            with c_m1:
                ...
                render_html("<div style='font-size:12px; color:var(--pink); font-family:Orbitron; margin-top:10px; margin-bottom:5px; text-align:center; font-weight:bold;'>[ 10-YEAR WIN RATE ] 赛季大运推演走势</div>")
                st.plotly_chart(f_trend, use_container_width=True, config={'displayModeBar': False})
            with c_m2:
                render_html("<div style='font-size:12px; color:var(--primary); font-family:Orbitron; margin-bottom:5px; text-align:center; font-weight:bold;'>[ COMBAT RADAR ] 六维面板雷达</div>")
                ...
                with e_psa:
                    ...
                    with c_e2:
                        if st.button("📸 压制 PSA 10 典藏卡砖 (MINT SLAB)", use_container_width=True):
                            # 控制海报装备显示数量避免超出
                            sk_h = "".join([f"<span style='background:rgba(255,255,255,0.15); border:1px solid rgba(255,255,255,0.3); color:#fff; padding:2px 6px; margin:2px; font-size:9px; display:inline-block; font-family:Fira Code; border-radius:2px;'>{s.split(' ')[1] if ' ' in s else s}</span>" for s in skills_list[:4]])
...
contract Karma_TCG_V110 is ERC721 {{
...
> WEAPON : {dm_wpn.split('】')[1] if '】' in dm_wpn else dm_wpn}
    ...
    def render_terminal():
        ...
        with st.form("inline_terminal", clear_on_submit=True, border=False):
            ...
            if sub_cmd and cmd_input:
                ...
                elif cmd_lower == '/rank': logs.append(f"<span style='color:var(--sp);'>[SYS] 您的当前战力(CP {final_cp:,})击败了全服 87.4% 的玩家。</span>")
                elif cmd_lower == '/clear': logs = ["> TERMINAL CLEARED."]
                elif cmd_lower == '/wuming': logs.append(f"<span style='color:var(--sp); font-weight:bold;'>[EASTER EGG] 欢迎来到无名逆流的赛博神域。万物皆虚，唯代码永存。</span>")
